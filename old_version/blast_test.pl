...
use warnings;
use Storable;
#open FILE,"/2_disk/niutj/perl_programs/test.fasta" or die "cannot open file,$!";
...
#	open FILE1,"/2_disk/chenyr/libfiles1/$word" ;#or die "cannot open the file: $word,$!";
    open FILE1,"/2_disk/niutj/make_lib2/$word";
    my $location=<FILE1>;
...
#	print "key1:$key1\n";
	foreach my $key2(keys %{$diag{$key1}}){
	    ...
	    if($diag{$key1}{$key2}>$key_value){
			...
			open FILE2,"/2_disk/niutj/hg19_splice/$key1.txt" or die "cannot open file:$key1,$!";
			my $line=<FILE2>;
#			print "\$key2:$key2\n";
			my $seq=substr($line,$key2-1,$len+10);
			print "\$seq2:$seq\n";
...
sub alignment{
	...
	printf " query_seq: %s\n",$align1;
	printf "genome_seq: %s\n",$align2;
	printf "Identity: %d/%d(%.2f%%)\n",$equal_num,length($align1),$identity;
