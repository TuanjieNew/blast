#!/usr/bin/perl
#file name:blast_test2.pl
use strict;
use warnings;
use Storable;
open FILE,"homo_search.fasta" or die "cannot open file";
my $my_lines;
while( my $line=<FILE>){
...
while(my ($index,$word)=each @search){
	...
	my $hash_ref=retrieve("./lib_test/hg_19.txt");
	my %hash=\$hash_ref;
	my $location=$hash{$word};
...
foreach my $key1(keys (%diag)){
	...
	foreach my $key2(keys %{$diag{$key1}}){
	    ...
	    if($diag{$key1}{$key2}>$key_value){
			...
			open FILE2,"./genome_splice/$key1" or die "cannot open file:$key1";
			my $line=<FILE2>;
			print "\$key2:$key2\n";
			my $seq=substr($line,$key2,$len);
			print "\$seq2:$seq\n";
